...
-- `ssh -N -L` tunnels itself, in code -- no :DbeeTunnel typing required.
-- Add/edit connections in `connections` below; each entry can carry an
-- optional `tunnel` table. A connection's tunnel is only opened lazily,
-- the first time that connection is selected in the UI (so picking a
-- connection with no `tunnel`, e.g. a local one, never spawns ssh for
-- some other connection's tunnel). Tunnels sharing the same local_port
-- are deduped and opened once.
--
...
local connections = {
	...
	{
		name = 'settle-local-runner',
		type = 'postgres',
		url = 'postgres://postgres:postgres@localhost:5453/settle_db?sslmode=disable'
...
local connections_by_name = {} ---@type table<string, DbeeConnectionSpec>
for _, conn in ipairs(connections) do
	connections_by_name[conn.name] = conn
end

---Opens the tunnel (if any) for the connection that was just selected, so
---connections without a `tunnel` -- e.g. the local one -- never pay for an
---ssh attempt, and only the tunnel actually in use gets started.
---@param conn_id string
local function open_tunnel_for_connection(conn_id)
	local ok, params = pcall(require('dbee').api.core.connection_get_params, conn_id)
	if not ok or not params then return end

	local conn = connections_by_name[params.name]
	if not conn or not conn.tunnel then return end

	local was_open = tunnel_jobs[conn.tunnel.local_port] ~= nil
	open_tunnel(conn.tunnel)
	-- Give ssh a beat to actually establish the forward before dbee's
	-- connection (pointed at localhost:<port>) gets used. Cheap and good
	-- enough in practice; if a first query still races the handshake, just
	-- retry it.
	if not was_open and tunnel_jobs[conn.tunnel.local_port] then vim.wait(600) end
end
...
return {
	...
	config = function()
		...
		-- nvim-dbee's backend also fires "current_connection_changed" as a
		-- side effect of *registering* each connection at startup (it makes
		-- the last-created connection "current" internally, regardless of
		-- what the user picks) -- not just on a real click in the drawer.
		-- Ignore that opening burst so a plain `:Dbee` doesn't itself try
		-- the last-registered connection's tunnel; only react once the UI
		-- has had time to come up, when the event can only mean a real
		-- selection.
		local ready = false
		vim.defer_fn(function() ready = true end, 500)

		require('dbee').api.core.register_event_listener('current_connection_changed', function(data)
			if not ready then return end
			open_tunnel_for_connection(data.conn_id)
		end)
	end,
}
